  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButtonLocation: FloatingActionButtonLocation
          .centerDocked, // adds a notch to a bottom app bar to fit a floating action button regardless of the floating action buttons size
    );
  }
}
    ...
    return Consumer<EmailStore>(
      builder: (context, model, child) {
        ...
        return _FadeThroughTransitionSwitcher(
          ...
          child: drawerVisible
              ? Align(
                  key: UniqueKey(), // so the framework can differentiate between the two Align widgets
                ...
                return ListTile(
                  ...
                  title: Text(
                    ...
                    style: theme.textTheme.bodyMedium!.copyWith(
                      color:
                          destination.name == currentlySelectedInbox ? theme.colorScheme.secondary : ReplyColors.white50.withOpacity(0.64),
                    ),
                  ),
                );
              },
    ...
    return Selector<EmailStore, bool>(
      ...
      builder: (context, onMailView, child) {
        ...
        final fabSwitcher = _FadeThroughTransitionSwitcher(
          fillColor: Colors.transparent, // no background between elements when transitioning
          child: onMailView
              ? Icon(
                  key:
                      fabKey, //  UniqueKey allows the framework to differentiate between both icons. Without a unique key, there would be no transition between the icons because the framework will simply give the widget new parameters instead of rebuilding it.
                  Icons.reply_all,
                  color: Colors.black,
                )
              : const Icon(
        ...
        return OpenContainer(
          ...
          //?? note: openBuilder handles its own routing
          openBuilder: (context, closedContainer) => const ComposePage(),
          openColor: theme.cardColor,
          onClosed: (data) {
            emailStoreReadOnly.onCompose = false;
          },
          ...
          closedBuilder: (context, openContainer) {
            return Tooltip(
              ...
              child: InkWell(
                ...
                onTap: () {
                  emailStoreReadOnly.onCompose = true;
...
//   - the ToolTip message is shown when a user presses and holds a widget that is wrapped with a ToolTip widget

// PageTransitionSwitcher - Widget 

//   - transitions between an old child and a new child whenever the child changes

//   - the transition animation used is the one specified by the transitionBuider callback

//   - two separate transitions can be specified, an entry transition (animation) and a exit transition (secondaryAnimation)

//   - if the old child and new child are of the same type then a key is required to differentiate between the two

//?? TODO: understand when to and when not to emphasize a spatial or hierarchical relationship?